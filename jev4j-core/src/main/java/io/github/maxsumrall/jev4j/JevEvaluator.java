...
public final class JevEvaluator {
    ...
    public Evaluation<Jev.NoulAnswer> evaluateWithMetadata(
            String state, Jev.NoulQuestion question) {
    ...
    public <A> CompletableFuture<A> evaluateAsync(Jev.State state, Jev.Question<A> question) {
        ...
        return exchangeAsync(
                ...
                state,
                QUESTION_KEY,
                result -> prepared.cast(result.answer().get(0)));
    }
    ...
    // These callbacks complete the public result themselves; their dependent stages are not
    // results.
        ...
        CompletableFuture<R> result =
                new CompletableFuture<>() {
                    ...
                    public boolean cancel(boolean mayInterruptIfRunning) {
                        // Abort before publishing cancellation, which can run blocking caller
                        // callbacks.
        ...
        transport.whenComplete(
                (response, failure) -> {
                    ...
                    try {
                        ...
                        // Do not strand the public future when decoding/projection fails in a
                        // callback.
    ...
    private Evaluation<List<Object>> decode(
        ...
        try {
            ...
            if (usageNode == null || !usageNode.isObject())
                throw malformed("missing object 'usage'");
            Usage usage =
                    new Usage(
    ...
    private static <A> Prepared<A> prepare(Jev.Question<A> question) {
        ...
        if (question instanceof Jev.NoulQuestion noul) {
            return (Prepared<A>)
                    new Prepared<>(
                            questionNode(
                                    "noul", noul.instructions(), noulCriteria(noul.descriptions())),
                            answer -> {
    ...
    private static ObjectNode questionNode(String type, String instructions, JsonNode criteria) {
        ObjectNode node =
                JSON.createObjectNode().put("type", type).put("instructions", instructions);
    ...
    private static void requireType(JsonNode answer, String expected) {
        ...
        if (!expected.equals(actual))
            throw malformed("answer type does not match the question type");
    }
    ...
    private static long requiredLong(JsonNode node, String field) {
        ...
        if (value == null
                ...
                || value.longValue() < 0)
            throw malformed("missing or invalid integer '" + field + "'");
    ...
    public <A1, A2> Evaluation2<A1, A2> evaluate(
        ...
        Evaluation<List<Object>> result =
                exchange(List.of(prepared1, prepared2), state, "question1");
    ...
    public record Evaluation4<A1, A2, A3, A4>(
        ...
        public <R> R map(
            ...
            return Objects.requireNonNull(mapper, "mapper")
                    .apply(answer1, answer2, answer3, answer4);
        }
    }
    ...
    public record Evaluation5<A1, A2, A3, A4, A5>(
        ...
        public <R> R map(
                Function5<? super A1, ? super A2, ? super A3, ? super A4, ? super A5, ? extends R>
                        mapper) {
            return Objects.requireNonNull(mapper, "mapper")
    ...
    public <A1, A2, A3, A4, A5, A6, A7> Evaluation7<A1, A2, A3, A4, A5, A6, A7> evaluate(
        ...
        Evaluation<List<Object>> result =
                exchange(
                        List.of(
                                prepared1, prepared2, prepared3, prepared4, prepared5, prepared6,
                                prepared7),
                        state,
                        "question1");
